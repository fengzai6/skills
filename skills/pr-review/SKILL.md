...
先明确最终回复方式；用户已说明时不重复询问：

- **直接回复**：仅在当前对话输出 review。
- **远程回复**：完成后将 review 回复到 PR。

用户未明确选择时，先询问并等待；确认后再开始审查。
...
2. 直接回复模式到此结束。
3. 远程回复模式仅在第 1 步完成后执行；发布前再次确认工具与目标 PR。
4. 远程回复失败时保留完整 review 文本，并报告失败原因，不改成猜测性摘要。
