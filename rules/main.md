...
7. **禁止超出范围**:
   ...
   - 例外：若在实现过程中发现以下情况，先停笔，指出问题并给建议方案，等确认再写（这不算超出范围）：
     - **他方信息缺失**：应由他方提供的信息未到位，当前却用局部猜测或魔法数字在硬补（例：后端未给 hasMore/total，前端用 `list.length > 10` 当判断）。
     - **方案存在关键歧义**：当前任务存在多种合理解读，当前角色自行选了一种，但可能与他方预期不符。
     - **存在明显技术风险**：当前做法虽暂时能跑，但会导致功能错误或高返工成本。
   - 以下不提：代码风格、命名偏好、性能微优化、可维护性建议等——除非直接导致功能错误。
8. **遵循项目现有风格**:
   - 始终匹配项目现有的代码风格和约定，不要自作主张改变。
9. **保持实现简洁**:
   - 代码能简短实现就保持简短，不要为了"更好看"而写长代码或添加抽象。
10. **文档与注释**: 只写最终行为、约束、决策和结果；只写 non-obvious reason。不写过程、被否方案、intermediate attempts。
11. **git 操作以用户明确要求为准**:
    - 未获明确要求时不自动提交:任务完成后停在「准备完成」——汇报完成内容与验证结果,等待用户指令,不执行任何 git 写操作。
    - 「准备完成」≠「提交」:"任务完成"、"测试通过"、"搞定"等表述不构成提交意图,不得自动执行 `git add` / `git commit` / `git push`、建分支或开 PR。
    - 仅当用户明确说出提交意图(如「commit」「提交」「push」「建分支」「开 PR」)时才执行,并遵循 `/git-workflow`。
...
- 注释只写 non-obvious reason，禁止保留 intermediate attempts。
...
- **优先有行为复杂度的逻辑**:
  - 有测试时，优先聚焦有分支、循环、状态、错误处理或业务不变量的逻辑。
  - 不要因为「这是函数 / 纯函数 / 工具方法」就补测；也不要因为「这不是函数」就默认要测。
- **禁止在测试中重写业务逻辑**:
  - 测试必须直接引用被测实现，禁止在测试中重新实现业务逻辑。
- **断言须基于已确认的行为**:
  - 对不确定的业务逻辑，先向用户确认，不能凭猜测写断言。
- **简单逻辑默认不写测试**:
  - 判断对象是代码行为是否简单，不是「是不是函数」。直给映射、透传、委托、单行转换、无分支无循环无状态、纯展示/布局、配置声明、类型定义、re-export 等，不要写测试。
  - 测试若只能复述实现（改实现必改测试、几乎没有失败可能），视为简单逻辑，不写。
  - 有分支、循环、状态机、协议/金额/权限/安全路径，或用户明确要求，才写。
...
- 用户要求审查 PR、提供 PR 编号/URL 或要求对分支变更集做 review → `/pr-review`
